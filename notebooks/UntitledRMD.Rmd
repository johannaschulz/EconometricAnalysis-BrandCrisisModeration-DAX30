...
```{r}
# with controls
linreg_2_0 <- lm(VWAP ~ Crisis + StrongBrand + (Crisis*StrongBrand) + Sector + EBIT + TR, data = data)
summary(linreg_2_0)
```
```{r}
# with lagged
...
```{r}
# without controls first 
linreg_2_1 <- lm(VWAP ~ CPI + StrongBrand + (CPI*StrongBrand), data = data)
summary(linreg_2_1)
```
```{r}
# with controls  
linreg_2_1 <- lm(VWAP ~ CPI + StrongBrand + (CPI*StrongBrand) + Sector + EBIT + TR, data = data)
summary(linreg_2_1)
```
...
```
```{r}
linreg_3_0 <- lm(VWAP ~ Crisis + BrandRanking + (Crisis*BrandRanking), data = data)
summary(linreg_3_0)
```
...
```{r}
# 1. Nonzero expectation: Zero mean for errors -> Resest test
resettest(linreg_2_1) # significant, violated
```

```{r}
# 2. Homoscedasticity: Constant variance for errors -> Breusch Pagan Test & Goldfeld-Quandt Test
gqtest(linreg_2_1) # not significant, assumptions holds
```
...
shapiro.test(residuals(linreg_2_1)) # significant -> VIOLATED
```
...
```{r}
linreg_3_0 <- lm(VWAP ~ CPI + BrandRanking + (CPI*BrandRanking), data = subset_strongbrand)
summary(linreg_3_0)
```


```{r}
linreg_3_0 <- lm(VWAP ~ CPI + BrandRanking + (CPI*BrandRanking) + Sector + TR + EBIT, data = subset_strongbrand)
summary(linreg_3_0)
```
...
### Hypothesis 3 | Model 3.0 (without controls) - Model 3.1 (with controls)

```{r}
linreg_3_0 <- lm(VWAP ~ Crisis + BrandRanking, data = data)
summary(linreg_3_0)
```
